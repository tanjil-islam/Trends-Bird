...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    Promise.all([
      api.get('/categories?limit=200').catch(() => ({ data: [] })),
      api.get('/brands?limit=200').catch(() => ({ data: [] })),
      api.get('/attributes?limit=200').catch(() => ({ data: [] })),
      api.get(`/products/${id}`).catch((err) => { throw err; }),
    ]).then(([cats, brs, attrs, prod]) => {
      ...
      setForm({
        ...
        stock: p.stock ?? '', stockStatus: p.stockStatus || 'in_stock',
        weight: p.weight ?? '', active: p.active, featured: p.featured,
        brandId: p.brandId || '',
        categoryIds: p.categories?.map((c: any) => c.categoryId) || [],
      });

      if (p.media) {
        setProductMedia(p.media.filter((m: any) => !m.variantId).map((m: any) => ({
          mediaId: m.mediaId,
          isThumbnail: m.isThumbnail,
          attributeValueId: m.attributeValueId || undefined,
        })));
      }
      ...
      if (p.variants) {
        setVariants(p.variants.map((v: any) => ({
          ...
          weight: v.weight ?? '',
          // Filter down attribute values to ONLY include attributeValueId to prevent ValidationPipe errors
          attributeValues: (v.attributeValues || []).map((av: any) => ({ attributeValueId: av.attributeValueId })),
          media: (v.media || []).map((m: any) => ({ mediaId: m.mediaId })),
        })));
      }
  ...
  const handleMediaSelect = (mediaIds: string[]) => {
    if (showMediaSelector?.type === 'product') {
      const newMedia = mediaIds.map((id, index) => ({
        mediaId: id,
        isThumbnail: productMedia.length === 0 && index === 0,
      }));
    ...
    <div className="max-w-4xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex gap-3">
          ...
          <button type="submit" disabled={submitting} className="btn btn-primary">{submitting ? <span className="spinner" /> : 'Save Changes'}</button>
        </div>
      </form>
